...
const Header = () => {
  ...
  useEffect(() => {
    ...
    // Cleanup on unmount: ensure scrolling is restored
    return () => {
  ...
  // ('megaMenu?.servicesType', megaMenu?.servicesType)

  return (
    <div className="flex flex-col items-center  gap-2 fixed w-full z-50 h-[115px] md:h-[160px] bg-white">
      {/* Top Nav */}
      <section className="hidden md:block relative w-[90%] py-1 px-4 rounded-b-[10px] bg-black text-white">
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center justify-center gap-3">
            <p className="flex gap-[2px] text-[15px] mt-4">
              <HiPhone className="text-[17px] mr-2 mt-[2px] " />
              <span className="text-[15px]">Karachi :</span>
              <a href="tel:+9221111762544" className=" items-center min-h-[28px] px-3 hover:text-[#F39C12]">
                {" "}
                UAN +92-21-111-762-544
              </a>{" "}
              :
              <a href="tel:+922134156597" className=" items-center min-h-[28px] px-3 hover:text-[#F39C12]">
                +92-21-34156597
              </a>
            </p>
            <p className="flex gap-1 text-[15px] mt-3 ">
              <IoMdMail className="mt-[2px] text-[18px]" />
              <a
                href="mailto:info@pobkarachi.org"
                className=" items-center min-h-[28px] px-3 hover:text-[#F39C12]"
              >
                info@pobkarachi.org
              </a>{" "}
              :
              <a href="mailto:hr@pobtrust.org" className=" items-center min-h-[28px] px-3 hover:text-[#F39C12]">
                hr@pobtrust.org
              </a>
            </p>
          </div>
          <div className="flex justify-center space-x-4 text-white">
            ...
            <a href="https://www.linkedin.com/company/pob-eyehospital-karachi" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
              <FaLinkedin size={18} />
            </a>
          </div>
        </div>
      ...
      <section className="block md:hidden w-full flex">
        <Link href="/donation/" title="Donate Now to Support Trusted Charity Initiatives" className="w-full border border-r-white">
          <div className="cursor-pointer text-white bg-[#D68910] text-white hover:bg-[#BF790C]
 px-2 py-1 gap-1 flex items-center justify-center border-2 border-transparent outline-none">
            <div className="bg-white px-1 py-1 rounded-full relative w-6 h-6">
              <NextImage
                src="/hand.webp"
                alt="Ballot box with a ballot paper being inserted."
                fill
                className="object-contain"
              />
            </div>

            <p className="mx-1 font-inter font-semibold ">Donate Now</p>
          </div>
        </Link>
      </section>
      ...
      <nav className="w-[90%] md:w-[90%] flex justify-center items-center">
        <div className="hidden lg:flex h-full lg:justify-between w-[80%] lg:w-full items-center rounded-bl-[20px] rounded-br-[20px]">
                  ...
                  {megaMenu?.blogs?.map((blog) => (
                    <div key={blog.id} className="flex items-start gap-4 mb-4">
                      <img
                        ...
                        alt={blog?.alt_text || blog.blog_title || "Blog Post"}
          ...
          <div className="flex gap-2">
            <Link href="https://www.pobtrust.com/donation/" aria-label="Donation link" title="Donate Now to Support Trusted Charity Initiatives">
              <div className="w-[145px] cursor-pointer text-white bg-[#F39C12] px-2 py-1 gap-1 flex items-center justify-center border-2 border-transparent rounded-full font-bold">
                <div className="bg-white px-1 py-1 rounded-[50%]">
                  <img src="/hand.webp" alt="Ballot box with a ballot paper being inserted." className="w-6" />
                </div>
        ...
        {/* Mobile Menu */}
        <div
          className={`fixed inset-0 bg-black h-screen z-50 bg-opacity-50 transition-transform transform ${showNav ? "translate-x-0" : "translate-x-full"
            }`}
                            ...
                            megaMenu.servicesType.map((service, sIndex) => (
                              ...
                              >
                                {/* <img
                                  ...
                                  alt={service?.alt_text || service?.name || "Service Icon"}
                                  className="w-4 h-4"
                                /> */}
