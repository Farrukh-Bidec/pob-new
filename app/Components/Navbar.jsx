    ...
    <div className="flex flex-col items-center gap-2 fixed top-0 left-0 w-full z-50 h-[115px] md:h-[120px] bg-white">
      ...
      <nav className="w-[90%] md:w-[90%] flex justify-center items-center">
                ...
                <div key={index} className="relative cursor-pointer font-inter text-[15px] group">
                  {item.name === "Services" ? (
                    <div
                      ...
                      onMouseLeave={() => {
                        hideMenuTimeout.current = setTimeout(() => setShowMegaMenu(false), 300);
                      }}
                    >
                      <Link
                        href={item.link || "#"}
              ...
              <div className="flex gap-8 justify-between pl-10 py-8 px-20">
                ...
                <div className="w-1/3 bg-[#F6F6F6] rounded-lg p-6">
                  ...
                  <div className="flex gap-2 mt-4">
                    {["Donate Now", "Subscribe Now", "Join Now"].map((btn, i) => (
                      <button
                        key={i}
                        onClick={btn === "Donate Now" ? () => { handleDonateClick(); setShowMegaMenu(false); } : undefined}
        ...
        <div className="flex justify-start w-full pl-2 items-center lg:hidden">
          <div className="lg:hidden text-black cursor-pointer right-[35px] absolute top-16" onClick={toggleShowNavbar}>
        ...
        >
          <div className="w-2/3 relative z-40 bg-white h-screen p-4 ml-auto" onClick={(e) => e.stopPropagation()}>
            <button className="text-black text-[12px]" onClick={() => setShowNav(false)}>
              <IoMdClose size={24} />
            </button>
            <div className="space-y-4 mt-8 flex flex-col gap-2">
              {mobileNavItems.map((item, index) => (
                <div key={index} className="text-black font-newsLetter border-b border-gray-500">
                  {item.name === "Services" ? (
                    <>
                      <div onClick={() => setShowServiceDropdown(!showServiceDropdown)} className="flex justify-between items-center py-1 cursor-pointer">
                        <span>Services</span>
                      ...
                      {showServiceDropdown && (
                        <ul className="pl-4 pt-2 space-y-2 h-36 overflow-auto">
                          {megaMenu?.servicesType?.map((service, sIndex) => (
                            <Link key={sIndex} href={`/${service.slug}`} onClick={() => setShowNav(false)} className="block text-sm text-gray-800">
                              {service.name}
                            </Link>
                          ))}
                        </ul>
                    ...
                    <>
                      <div onClick={() => setShowHelpDropdown(!showHelpDropdown)} className="flex justify-between items-center py-1 cursor-pointer">
                        <span>{item.name}</span>
                      ...
                      {showHelpDropdown && (
                        <ul className="pl-4 pt-2 space-y-2">
                          {item.submenu.map((sub, subIndex) => (
                            <Link key={subIndex} href={sub.link} onClick={() => setShowNav(false)} className="block text-sm text-gray-800">
                              {sub.name}
                            </Link>
                          ))}
                        </ul>
                    ...
                    <Link href={item.link} onClick={() => setShowNav(false)} className="block py-1">
                      {item.name}
                    </Link>
                  )}
                </div>
