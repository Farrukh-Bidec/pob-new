...
const Faqs = ({ faqs }) => {
  ...
  // If FAQs are loaded asynchronously, open the first item by default
  // when the data becomes available and no item is active yet.
  React.useEffect(() => {
    const hasFaqs = faqs?.[0]?.pages_faq_details?.length > 0;
    if (hasFaqs && activeIndex === null) setActiveIndex(0);
  }, [faqs, activeIndex]);
    ...
    <div className="flex justify-center flex-col md:flex-row items-center py-12 md:py-0 md:pt-10 md:items-start gap-4 bg-transparent w-full">
      <div className="flex justify-center flex-col md:flex-row items-center md:items-start md:w-[80%]">
        ...
        <div className="flex flex-col pt-4 md:w-[40%] w-[100%] text-center md:text-start">
          <h3 className="text-[#F39C12] uppercase text-lg font-Amaranth font-bold">
            {faqData?.top_heading}
          </h3>
          ...
          {faqData?.pages_faq_details?.map((faq, index) => (
            <div key={faq.id || index} className="w-full mb-2">
              ...
              <div className="transition duration-300">
                ...
                >
                  <p className="md:text-sm font-poppins text-sm text-black py-4">
                    {faq.question}
                  </p>
                  <span className="md:text-2xl text-xs px-4 text-black">
                    {activeIndex === index ? <IoIosArrowUp /> : <IoIosArrowDown />}
                  </span>
                </div>
                {activeIndex === index && (
                  <div className="py-2 text-black text-xs rounded-b-xl">
                    <p
