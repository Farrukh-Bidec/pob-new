...
const Section2 = ({ slug }) => {
  return (
    <div className="flex flex-col md:flex-row min-w-full pt-12 gap-4 items-center justify-center mb-10">
      ...
      <div>
        <Image
          src="/DiabeticService.webp"
          width={450}
          height={450}
          alt="Surgeon performing eye surgery using a microscope, wearing surgical attire and gloves."
      ...
      <div className="md:w-5/12 w-[90%] font-inter items-center md:items-start flex flex-col gap-2">
        <p className="uppercase text-[#F39C12] font-Amaranth text-center md:text-start text-lg font-bold">
          Vitroretina
        </p>
        <h2 className="text-4xl max-w-xl font-Amaranth text-center md:text-start">
          Specialized Care For Retina Health
        </h2>
        <p className="text-[#777777] text-center md:text-start md:max-w-xl font-medium">
        ...
        <Link href="https://www.pobtrust.com/vitreoretina/" title="Learn More About Vitreoretina Services">
          <button className="mt-4 px-4 py-2 w-32 rounded-full text-[12px] font-inter text-white font-bold bg-[#28A745] hover:bg-white hover:text-[#28A745] transition-all duration-300 shadow-sm">
            View Details
          </button>
        </Link>
      </div>
